...
import {JobsWords} from "../../Data/JobsData"
...
function Jobs() {
    ...
    const nextSlide = () => {
        setCurrentIndex((prevIndex) =>
            prevIndex === JobsWords.length - 1 ? 0 : prevIndex + 1
        );
    };
    const prevSlide = () => {
        setCurrentIndex((prevIndex) =>
            prevIndex === 0 ? JobsWords.length - 1 : prevIndex - 1
        );
    };

    const getRandomIndex = () => Math.floor(Math.random() * JobsWords.length);
    
    const getRandomIndexes = (correctAnswer) => {
        const indexes = [];
        while (indexes.length < 3) {
            const randomIndex = Math.floor(Math.random() * JobsWords.length);
           ...
           {!showTestWindow && <div className="memorize-words">
                ...
                <div className="words-slider">
                    ...
                    <div className="word-card">
                        <img src={JobsWords[currentIndex].img} alt="Animal" />
                        <h2 className="en-w">{JobsWords[currentIndex].enW}</h2>
                        <h3 className="ar-w">{JobsWords[currentIndex].arW}</h3>
                        <p className="sentence">{JobsWords[currentIndex].enSen}</p>
                    </div>
            ...
            {showTestWindow && (
                ...
                <div className="test">
                        ...
                        <h1 className="question">
                            ماذا تعني كلمة - <b>{JobsWords[correctAnswer].arW}</b>
                        </h1>

                    <div className="choose-card">
                        {getRandomIndexes(correctAnswer).map((cardIndex, i) => (
                            <div className="word-card" key={i} onClick={()=>handleCardClick(cardIndex)} >
                                <img src={JobsWords[cardIndex].img} alt="Animal" />
                                <h2 className="en-w">{JobsWords[cardIndex].enW}</h2>
                                <p className="sentence">{JobsWords[cardIndex].enSen}</p>
                            </div>
                        ))}
                    </div>
                </div>
