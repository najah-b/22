...
import {FruitsWords} from "../../Data/FruitsData"
...
function Fruits() {
    ...
    const nextSlide = () => {
        setCurrentIndex((prevIndex) =>
            prevIndex === FruitsWords.length - 1 ? 0 : prevIndex + 1
        );
    };
    const prevSlide = () => {
        setCurrentIndex((prevIndex) =>
            prevIndex === 0 ? FruitsWords.length - 1 : prevIndex - 1
        );
    };

    const getRandomIndex = () => Math.floor(Math.random() * FruitsWords.length);
    
    const getRandomIndexes = (correctAnswer) => {
        const indexes = [];
        while (indexes.length < 3) {
            const randomIndex = Math.floor(Math.random() * FruitsWords.length);
           ...
           {!showTestWindow && <div className="memorize-words">
                ...
                <div className="words-slider">
                    ...
                    <div className="word-card">
                        <img src={FruitsWords[currentIndex].img} alt="Animal" />
                        <h2 className="en-w">{FruitsWords[currentIndex].enW}</h2>
                        <h3 className="ar-w">{FruitsWords[currentIndex].arW}</h3>
                        <p className="sentence">{FruitsWords[currentIndex].enSen}</p>
                    </div>
            ...
            {showTestWindow && (
                ...
                <div className="test">
                        ...
                        <h1 className="question">
                            ماذا تعني كلمة - <b>{FruitsWords[correctAnswer].arW}</b>
                        </h1>

                    <div className="choose-card">
                        {getRandomIndexes(correctAnswer).map((cardIndex, i) => (
                            <div className="word-card" key={i} onClick={()=>handleCardClick(cardIndex)} >
                                <img src={FruitsWords[cardIndex].img} alt="Animal" />
                                <h2 className="en-w">{FruitsWords[cardIndex].enW}</h2>
                                <p className="sentence">{FruitsWords[cardIndex].enSen}</p>
                            </div>
                        ))}
                    </div>
                </div>
