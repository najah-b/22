...
import {VerbsWords} from "../../Data/VerbsData"
...
function Verbs  () {
    ...
    const nextSlide = () => {
        setCurrentIndex((prevIndex) =>
            prevIndex === VerbsWords.length - 1 ? 0 : prevIndex + 1
        );
    };
    const prevSlide = () => {
        setCurrentIndex((prevIndex) =>
            prevIndex === 0 ? VerbsWords.length - 1 : prevIndex - 1
        );
    };

    const getRandomIndex = () => Math.floor(Math.random() * VerbsWords.length);
    
    const getRandomIndexes = (correctAnswer) => {
        const indexes = [];
        while (indexes.length < 3) {
            const randomIndex = Math.floor(Math.random() * VerbsWords.length);
           ...
           {!showTestWindow && <div className="memorize-words">
                ...
                <div className="words-slider">
                    ...
                    <div className="word-card">
                        <img src={VerbsWords[currentIndex].img} alt="Animal" />
                        <h2 className="en-w">{VerbsWords[currentIndex].enW}</h2>
                        <h3 className="ar-w">{VerbsWords[currentIndex].arW}</h3>
                        <p className="sentence">{VerbsWords[currentIndex].enSen}</p>
                    </div>
            ...
            {showTestWindow && (
                ...
                <div className="test">
                        ...
                        <h1 className="question">
                            ماذا تعني كلمة - <b>{VerbsWords[correctAnswer].arW}</b>
                        </h1>

                    <div className="choose-card">
                        {getRandomIndexes(correctAnswer).map((cardIndex, i) => (
                            <div className="word-card" key={i} onClick={()=>handleCardClick(cardIndex)} >
                                <img src={VerbsWords[cardIndex].img} alt="Animal" />
                                <h2 className="en-w">{VerbsWords[cardIndex].enW}</h2>
                                <p className="sentence">{VerbsWords[cardIndex].enSen}</p>
                            </div>
                        ))}
                    </div>
                </div>
